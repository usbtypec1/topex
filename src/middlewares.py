...
# class StateClearMw(BaseMiddleware):
#     async def __call__(
#         self,
...
#         data: Dict[str, Any]
#     ) -> Any:
#         state: FSMContext = data.get("state")  # Извлекаем state из data

#         if not state:
#             return await handler(event, data)  # Если state нет, просто вызываем handler
